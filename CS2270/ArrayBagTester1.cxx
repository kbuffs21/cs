...
int main()
	{
		...
		aBag.add("tamale");
		aBag.add("taco");
		aBag.add( -2270 );//test addition, add integer
		abag.add("taco",25);//test addition add string with int
		...
		if (aBag.contains( -2270 ) == true)
			cout << "Bag shouldn't contain this" << endl;
		else
			cout << "integer not found; that is good" << endl;
		
		if (aBag.contains( 25 ) == true)
			cout << "Bag shouldn't contain this" << endl;
		else
			cout << "integer not found; that is good" << endl;
		...
		aBag.remove(aBad.add("zero"));//test addition, two nullifying functions
		
		// here we check if the ArrayBag knows its own size after a non-event removal and test addition
		if (aBag.getCurrentSize() != 3)
		...
		// test addition, add entries with loop
		for (int i = 0; i < 10; ++i)
			aBag.add("tentens");
		if (aBag.getCurrentSize() != 10)
			cout << "Bag after loop add has wrong size; that is not so good" << endl;
		else
			cout << "Bag after loop add has right size; that is good" << endl;	
		...
		//test addition, remove from empty bag
		aBag.remove("tamale");
			
		// check that remove has left the size correct
		if (aBag.getCurrentSize() != 0)
			cout << "removed from empty bag; that is not so good" << endl;
		else
			cout << "did not remove from empty bag; that is good" << endl;
			
		// test addition, attempt to change capacity with public method
		int n = aBag.getCapacity(); 	
		aBag.getCapacity(1++);
		int m = aBag.getCapacity();
		if (n != m)
			cout << "Size of bag is incorrect; that is not so good" << endl;
		else
			cout << "Size of bag is correct; that is good" << endl;
	}
