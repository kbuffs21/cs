    ...
    unsigned int Aligner::match()
    {
		for (unsigned int q = 1; q < rows + 1; ++q)
		{
			for (unsigned int r = 1; r < cols + 1; ++r)
			{
				...
				{
					if ((1 + matches[q-1][r-1]) >= matches[q-1][r] &&  (1 + matches[q-1][r-1]) >= matches[q][r-1])
					{
						matches[q][r] = (1 + matches[q-1][r-1]);
					}
					else if (matches[q-1][r] > (1 + matches[q-1][r-1]) &&  (1 + matches[q-1][r-1]) >= matches[q][r-1])
					{
				...
				{
					if (matches[q-1][r] >= matches[q][r-1])
					{
						matches[q][r] = matches[q-1][r];
					}
					else
		...
		/*function backtrack(matches[q][r], sequence1[q], sequence2[r], i, j)
			if i = 0 or j = 0
				return ""
			else if  sequence1[i] = sequence2[j]
				return backtrack(matches[q][r], sequence1[q], sequence2[r], --i, --j) + sequence1[i]
			else
				if matches[q][r-1] > matches[q-][r]
					return backtrack(matches[q][r], sequence1[q], sequence2[r], i, --j)
				else
					return backtrack(matches[q][r], sequence1[q], sequence2[r], --i, j)
		*/
